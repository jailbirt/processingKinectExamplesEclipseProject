...
public class kinectUtilities extends PApplet {
	
	kinect parent;
	
	public kinectUtilities (kinect kinect){
	 parent=kinect;
	}
	...
	void calculateLimits(int[] depthImg, PVector[] map3D){
		  ...
		  parent.xmin = (float) (xMin - 0.1*(xMax - xMin));
		  parent.xmax = (float) (xMax + 0.1*(xMax - xMin));
		  parent.ymin = (float) (yMin - 0.1*(yMax - yMin));
		  parent.ymax = (float) (yMax + 0.1*(yMax - yMin));
		  parent.zmin = (float) (zMin - 0.5*(zMax - zMin));
		  parent.zmax = (float) (zMax + 0.1*(zMax - zMin));
		}  

		int[] resizeDepth(int[] depthImg, int n){
		  int xSizeOrig = parent.context.depthWidth();
		  int ySizeOrig = parent.context.depthHeight();
