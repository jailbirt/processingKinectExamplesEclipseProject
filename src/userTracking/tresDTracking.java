...
//	protected simpleTracking thisO;//y singleton ?  y syncronized ?
	...
	public void setup() {
	    ...
	    kinect = new SimpleOpenNI(this,SimpleOpenNI.RUN_MODE_MULTI_THREADED); //MULTI_THREAD openni y nite .
		kinect.enableDepth();
	...
	public void draw() {
		...
		if (userList.size() > 0) {// if we found any users
			...
			if (kinect.isTrackingSkeleton(userId)) {
				...
				float confidence =	kinect.getJointOrientationSkeleton(userId,SimpleOpenNI.SKEL_TORSO,
						            orientation); //2 Agrego data de orientación a la matrix orientation.
				
//Trabajo denuevo sobre las manos.
				//Tomo la posicion de ambas.
				PVector leftHand = new PVector();
				kinect.getJointPositionSkeleton(userId,SimpleOpenNI.SKEL_LEFT_HAND,leftHand); //5
				PVector rightHand = new PVector();
				kinect.getJointPositionSkeleton(userId,SimpleOpenNI.SKEL_RIGHT_HAND,rightHand); //6
				// subtract right from left hand to turn leftHand into a vector representing
				// the difference between them
				leftHand.sub(rightHand);
				// convert leftHand to a unit vector
				leftHand.normalize();//7 dejo el vector unitario para la direción.;//Para calcular desp -9
                ...
                //the dot product tells us the angle between the vectors
				...
				// rotate angle amount around all axis, mantiene alineado los sistemas de coordenadas.
